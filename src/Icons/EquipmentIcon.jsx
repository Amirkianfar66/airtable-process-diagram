...
export default function EquipmentIcon({ id, data }) {
    ...
    const TypeSVG = TYPE_SVGS[data?.Type];
    ...
    return (
        ...
        >
            <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left', position: 'relative', width: 150, height: 150 }}>
                {/* Render type-specific SVG if exists, else fallback */}
                {TypeSVG ? (
                    <img src={TypeSVG} alt={data?.Type} style={{ width: '100%', height: '100%' }} />
                ) : (
                    <svg width="150" height="150">
                        <rect x="0" y="0" width="150" height="150" fill="green" />
                        <text x="75" y="75" fontSize="16" textAnchor="middle" fill="white">EQ</text>
                    </svg>
                )}
