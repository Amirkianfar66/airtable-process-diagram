...
export default function EquipmentIcon({ id, data }) {
    ...
    const [icon, setIcon] = useState(TYPE_ICONS[data?.item?.Type] || null);
    ...
    useEffect(() => {
        // Update icon whenever Type changes
        setIcon(TYPE_ICONS[data?.item?.Type] || null);
    }, [data?.item?.Type]);
    ...
    return (
        ...
        >
            ...
            >
                {/* Render dynamic icon */}
                {icon ? <img src={icon} alt={data?.item?.Type} style={{ width: '100%', height: '100%' }} /> : (
