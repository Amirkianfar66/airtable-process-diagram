...
export default function EquipmentIcon({ id, data }) {
    ...
    const [IconComponent, setIconComponent] = useState(TYPE_ICONS[data?.item?.Type] || null);
    ...
    useEffect(() => {
        // Update icon component whenever Type changes
        setIconComponent(TYPE_ICONS[data?.item?.Type] || null);
    }, [data?.item?.Type]);
    ...
    const handleMouseEnter = () => { if (timeoutRef.current) clearTimeout(timeoutRef.current); setHovered(true); };
    const handleMouseLeave = () => { timeoutRef.current = setTimeout(() => setHovered(false), 2000); };

    useEffect(() => () => clearTimeout(timeoutRef.current), []);

    return (
        <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={{ position: 'relative', width: 200, height: 200, textAlign: 'center', userSelect: 'none' }}>
            <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left', position: 'relative', width: 150, height: 150 }}>
                {IconComponent ? <IconComponent width="100%" height="100%" /> : (
                    <svg width="200" height="200" viewBox="0 0 200 200" style={{ pointerEvents: 'none', display: 'block', margin: '0 auto' }}>
