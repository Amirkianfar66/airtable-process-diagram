...
export default async function handler(req, res) {
    try {
        ...
        const conversationalRegex = /\b(hi|hello|hey|how are you|thanks|thank you)\b/i;
        if (conversationalRegex.test(description.trim())) {
            return res.json({
                mode: "chat",
                explanation: `👋 Hi there! How can I help with your process diagram?`,
                parsed: {},
                connection: null
            });
        }
...
`;
        ...
        return res.json({ mode: "structured", explanation, parsed, connection });
    } catch (err) {
