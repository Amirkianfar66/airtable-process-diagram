...
`;
    ...
    try {
        ...
        try {
            ...
            // 🔹 Step 1: extract items & connections
            let rawItems = [];
            let rawConnections = [];
            ...
            if (parsed?.orders && Array.isArray(parsed.orders)) {
                ...
                rawConnections = rawItems.flatMap(i => i.Connections || []);
            }

            // 🔹 Step 2: normalize items
            const itemsArray = rawItems.map((item, idx) => ({
                mode: "structured",
                Name: (item.Name || `Item${idx + 1}`).toString().trim(),
            ...
            itemsArray.forEach(item => {
                ...
                item.Connections = connectionResolved
                    .filter(c =>
                        (c.from?.toString().toLowerCase() === lowerName) ||
                        (c.from?.toString() === itemCode)
                    )
                    .map(c => c.to);
            });


            // 🔹 Step 7: return clean structure
            return {
                parsed: itemsArray,
                connectionResolved,
